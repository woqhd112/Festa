...
<script type="text/javascript">
	...
	if(url.indexOf('gp_report')>0){
		window.location.href='${root}empty';
	}
	console.log(url.indexOf('report'));
	$(document).ready(function(){
		...
		var proid = $('#proid').val();
		$('#form_data>input[type=hidden]').eq(3).val(grnum);
		$('#form_data>input[type=hidden]').eq(4).val(pronum);
		$('#form_data>input[type=hidden]').eq(5).val(proname);
		$('#form_data>input[type=hidden]').eq(6).val(proid);
...
<div class="report_wrap pop_wrap">
	...
	<form class="comm_form" id="form_data" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="pronum_sync" value="">
		<input type="hidden" name="rltarget" value="">
...
<div id="excess1" class="fstPop">
	<div class="confirm_wrap pop_wrap">
		<p class="pop_tit">신고내용은 500자 이상 입력할수 없습니다.</p>
		<ul class="comm_buttons">
